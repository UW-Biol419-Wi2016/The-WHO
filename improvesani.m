...
for i = 1:numel(sanitationdouble(:,2)),
    if sanitationdouble(i,1) == 1990,
        sanitationy(n,1) = sanitationdouble(i,7);
        n = n+1;
    end;
    
    if sanitationdouble(i,1) == 2000,
        sanitationy(m,2) = sanitationdouble(i,7);
        m = m+1;
    end;
    
    if sanitationdouble(i,1) == 2015,
        sanitationy(p,3) = sanitationdouble(i,7);
        p = p+1;
    end;
end;

savg1990 = nanmean(sanitationy(:,1))
savg2000 = nanmean(sanitationy(:,2))
savg2015 = nanmean(sanitationy(:,3))

figure;
hold on
plot([sanitationx(1,1),sanitationx(1,2),sanitationx(1,3)],[savg1990,savg2000,savg2015],'k-o')
plot(sanitationx, sanitationy, 'x')
legend('average percent with improved sanitation facilities')
title('Percentages of Improved Sanitation Facilities in Every Country')
xlabel('Year')
